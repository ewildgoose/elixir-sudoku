defmodule Sudoku.Strategy do
  ...
  * {:backtrack, _}
  ...
  def find_naked_tuples(board) do
    ...
    naked_tuples =
      (find_naked_tuples_in_group(unsolved, &group_by_y/1, :row) ++
  ...
  def find_grid_candidates_in_group(squares, candidate, group_fn_1, group_fn_2, label) do
    ...
    |> Enum.filter_map(
      fn {num_rows, squares} -> num_rows == count_groups(squares, group_fn_2) end,
  ...
  @doc """
  Is the board position invalid? Do we need to backtrack?
  """
  ...
  *
  ...
  @doc """
  Return a list of squares that aren't yet solved
  """
  ...
  @doc """
  Given a group of squares, see if they are all contained in a single box
  """
  def squares_in_single_box?(squares), do: 1 == count_groups(squares, &group_by_box/1)

  @doc """
  Given a group of squares, see if they are all contained in a single col
  """
  def squares_in_single_col?(squares), do: 1 == count_groups(squares, &group_by_x/1)

  @doc """
  Given a group of squares, see if they are all contained in a single row
  """
  ...
  @doc false
  # Some strategies generate inferences that whilst true, may not advance the solution, eg :grid_analysis
