defmodule SudokuStrategyTest do
  ...
  test "naked singles" do
    step = Board.new("246975138589316274371040695498621753132754986657830421724183569865492317913567842")
    |> Strategy.find_naked_singles

    assert( step == {:naked_singles,
  ...
  test "naked singles 2" do
    ...
    step = Board.new("246970038580316274370040690498620753132754986607030421724183569865492317913567842")
    |> Strategy.find_naked_singles
  ...
  test "locked candidates" do
    step = @locked_candidates_1
    |> Board.new
    |> Strategy.find_locked_candidates

    assert ( step == {:locked_candidates,
                       [box: [%Sudoku.Square{box_x: 0, box_y: 0, candidates: MapSet.new([3]), solution: nil, x: 2, y: 2},
                              %Sudoku.Square{box_x: 0, box_y: 0, candidates: MapSet.new([3]), solution: nil, x: 1, y: 2}],
                        row: [%Sudoku.Square{box_x: 2, box_y: 0, candidates: MapSet.new([3]), solution: nil, x: 6, y: 1},
                              %Sudoku.Square{box_x: 2, box_y: 0, candidates: MapSet.new([3]), solution: nil, x: 8, y: 1}],
                        row: [%Sudoku.Square{box_x: 0, box_y: 1, candidates: MapSet.new([9]), solution: nil, x: 2, y: 4},
